...
func LoadServicesConfig(v *viper.Viper) (*servicemanager.TopLevelConfig, error) {
...
func LoadDeploymentConfig(logger zerolog.Logger, v *viper.Viper) (*DeployerConfig, error) {
	...
	if cfg.DirectorServiceURL == "" {
		if v.IsSet("director_service_url") {
			cfg.DirectorServiceURL = v.GetString("director_service_url")
		}
	}
