...
func main() {
	...
	pflag.StringVar(&deployerCfgPath, "deploy", "deployment.yaml", "source path for the deployment configuration file")

	var servicesDefPath string
	pflag.StringVar(&servicesDefPath, "services", "services.yaml", "source path for the services definition file")

	var servicesDirectorURL string
	pflag.StringVar(&servicesDirectorURL, "service-director-url", "", "if serviceDirector exists enter its URL")
	...
	log.Info().Str("serviceDirectorURL", servicesDirectorURL).Msg("Flag value")
	// --- END DEBUGGING ---
	...
	if err != nil {
		log.Fatal().Err(err).Msg("BindPFlags failed")
	} // Bind AFTER parsing
	...
	// 3. Load coordinater configuration using its loader.
	// This assumes config.LoadConfig (in coordinater/config/loader.go) handles its own flag parsing (with pflag) internally.
	deployerCfg, err := config.LoadDeploymentConfig(logger, v) // Calling without passing deployerCfgPath, as per user's main.go
	if err != nil {
		log.Fatal().Err(err).Msg("Failed to load coordinater configuration")
	}

	servicesCfg, err := config.LoadServicesConfig(v)
	if err != nil {
		log.Fatal().Err(err).Msg("Failed to load coordinater configuration")
	}
	...
	case "deploy":
		...
		err := coordinater.CoordinateDeployment(ctx, servicesCfg, deployerCfg)
		if err != nil {
			return
		}
