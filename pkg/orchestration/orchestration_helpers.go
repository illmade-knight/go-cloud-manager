...
// verifyPrerequisites is the main entry point for pre-flight checks.
func verifyPrerequisites(ctx context.Context, arch *servicemanager.MicroserviceArchitecture) error {
	...
	if err := verifyRegion(ctx, arClient, arch); err != nil {
		return err
	}
	...
	if repoName != "" {
		if err := ensureArtifactRegistryRepositoryExists(ctx, arClient, arch.ProjectID, arch.Region, repoName); err != nil {
			return err
		}
	}
	return nil
...
// verifyRegion checks that the region specified in the architecture is a valid location.
// It does this by making a lightweight API call to the regional endpoint.
func verifyRegion(ctx context.Context, client *artifactregistry.Client, arch *servicemanager.MicroserviceArchitecture) error {
	...
	// This is a lightweight and reliable way to check the endpoint.
	...
	// If the error is anything other than "Not Found", it means the region is likely invalid.
	...
	// If the error was NotFound or there was no error, the region itself is valid.
	log.Info().Str("region", arch.Region).Msg("Region is valid.")
	return nil
}

// ensureArtifactRegistryRepositoryExists creates the repo if it's missing.
